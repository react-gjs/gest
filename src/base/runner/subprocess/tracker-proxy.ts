...
export class ProgressTrackerProxy {
  suiteProgress(progressUpdate: SuiteUpdateParams) {
    ...
    if (progressUpdate.error) {
      progressUpdate.error.thrown = serializeError(progressUpdate.error.thrown);
    }

    Subprocess!.invoke.suiteProgress(progressUpdate).catch(printError);
  }
  ...
  finish() {
    Subprocess!.invoke.finish().catch(printError);
  }
}
