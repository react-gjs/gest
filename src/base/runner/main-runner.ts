...
class WorkerApi {
  ...
  private api: MainProcessApi = {
    ...
    finish: () => {
      ...
      this.currentTracker.finish(this.currentSuiteID);
    },
    sendLog: (type, data) => {
...
export class MainRunner {
  ...
  private async runTestOnSubprocess(id: symbol, filepaths: TestFilepaths) {
    ...
    return new Promise<void>(async (resolve, reject) => {
      ...
      worker.invoke
        .runSuite(filepaths, this.options, this.config.defaultTimeoutThreshold)
        .catch(reject);
    });
  }

  private async runTestOnMainThread(id: symbol, filepaths: TestFilepaths) {
