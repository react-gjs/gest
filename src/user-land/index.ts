...
type AssertAllFn<
  A extends any[],
  K extends Exclude<keyof ExpectAsserts, "not">
> = ReturnType<ExpectAsserts[K]> extends Promise<any> ? Promise<void> : void;
  ...
  return {
    assert(...args): any {
      ...
      if (r instanceof Promise) {
        return assertionHandlers.async(r, negate, calledFrom).then(() => this);
      } else {
...
export const defineMatcher = (matcherName: string, matcher: Assert) => {
